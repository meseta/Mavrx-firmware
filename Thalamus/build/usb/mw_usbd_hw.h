...
typedef struct USBD_HW_API
{
   ...
   * 
   *  This fuction is called by application layer before calling USBROM->hw->Init(), to allocate memory used 
   ...
   *  This function is called internally by USBROM->hw->init() and from reset event.
   *  
   *  \param[in]  hUsb Handle to the USB device stack. 
   *  \return Nothing.
   */
   ...
   *  of USBD_API_INIT_PARAM_T structure before calling USBROM->hw->Init() routine.
   *  Certain USB device controllers needed to keep some clocks always on to generate 
   *  resume signaling through USBROM->hw->WakeUp(). This hook is provided to support 
