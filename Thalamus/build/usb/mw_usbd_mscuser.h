...
typedef struct USBD_MSC_API
{
   ...
   * 
   *  This function is called by application layer before calling USBROM->msc->Init(), to allocate memory used 
...
typedef struct _MSC_CTRL_T
{
  ...
  /*ALIGNED(4)*/ uint8_t  BulkBuf[USB_MAX_BULK_PACKET]; /* Bulk In/Out Buffer */
