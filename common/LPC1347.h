...
#ifndef __LPC1347_H__
#define __LPC1347_H__
...
static inline void IRQEnable(IRQn_Type IRQn) { NVIC->ISER[((uint32_t)(IRQn) >> 5)] = (1 << (uint32_t)(IRQn)); }
static inline void IRQDisable(IRQn_Type IRQn) { NVIC->ICER[((uint32_t)(IRQn) >> 5)] = (1 << (uint32_t)(IRQn)); }
static inline void IRQClear(IRQn_Type IRQn) { NVIC->ICPR[((uint32_t)(IRQn) >> 5)] = (1 << (uint32_t)(IRQn)); }
static inline void IRQPriority(IRQn_Type IRQn, uint32_t priority) {
    if(IRQn < 0) SCB->SHP[((uint32_t)(IRQn) - 4) & 0xF] = (priority << 4) & 0xff; // core interrupts
    else NVIC->IP[(uint32_t)(IRQn)] = (priority << 4) & 0xff; // peripheral interrupts
}
...
static inline void __set_Stack(unsigned stack_top) {
    asm volatile("MOV sp, r0");
    //asm volatile ("MOV sp, 0x10002000");
    asm volatile("BX lr");
}

/*asm void __set_Stack(unsigned stack_top) {
   MOV sp,r0
   BX lr
}*/
